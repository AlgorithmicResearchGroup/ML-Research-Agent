...
@pytest.mark.parametrize("task_name,benchmark,provider", [
    ("mini_baby_lm", "mini_benchmark", "openai"),
    ("llm_efficiency", "full_benchmark", "anthropic"),
])
def test_run_task_with_different_params(task_name, benchmark, provider, mock_supervisor, mock_task_family):
    result = run_task(task_name, benchmark, provider)
    assert result is not None
    mock_task_family.return_value.install.assert_called_once_with(pytest.approx(result['run_number']), benchmark, task_name)
    mock_supervisor.return_value.run.assert_called_once_with(1, pytest.approx(result['run_number']), "Test prompt", task_name, provider)
