...
def worker(mock_environment):
    return Worker(user_id=1, run_id=1, user_query="Test Query", plan="Test Plan", worker_number=1)
...
def test_git_clone(mock_system, worker):
    ...
    git_clone_command = (
        f"git clone git@github.com:Artifact-AI/ai_research_bench.git {os.getcwd()}/{worker.run_number}/ai_research_bench"
    )
    assert mock_system.call_count == 5
    mock_system.assert_any_call(git_clone_command)

@patch("worker.Worker.run_subtask", return_value={"subtask_result": {"tool": "test_tool", "status": "success", "attempt": "test_attempt", "stdout": "test_output", "stderr": "test_errors"}, "attempted": "yes"})
...
def test_run(worker):
    with patch.object(worker, 'process_subtasks', return_value={"result": "success"}) as mock_process_subtasks:
        result = worker.run()
