...
def test_generate_plan_openai(mock_openai_create, supervisor):
    ...
    mock_response.choices[0].message.tool_calls[0].function.arguments = json.dumps({"plan": ["Step 1", "Step 2"]})
...
def test_parse_chat_response_to_subtasks_openai(supervisor):
    ...
    mock_chat_response.choices[0].message.tool_calls[0].function.arguments = json.dumps({"plan": ["Step 1", "Step 2"]})
...
def test_parse_chat_response_to_subtasks_anthropic(supervisor):
    supervisor.agent_model = "anthropic"
    mock_chat_response = MagicMock()
    mock_chat_response.content = [{"input": {"plans": [{"plan": ["Step 1", "Step 2"]}]}}]
...
@patch.object(Supervisor, 'parse_chat_response_to_subtasks', return_value=[[{"Subtask": "Step 1"}, {"Subtask": "Step 2"}]])
def test_run(mock_parse_chat_response_to_subtasks, mock_generate_plan, mock_worker, supervisor):
    ...
    mock_worker.assert_called_once_with(user_id, run_id, user_query=task, plan='Plan 0 --->\nStep 1\nStep 2\n\n', worker_number=1)
